...
final class OpProgram {
  ...
  static long[] run(
    ...
    for (int instructionPtr = 0; (0 <= instructionPtr) && (instructionPtr < instructions.size()); ++instructionPtr) {
      registers[instructionRegister] = instructionPtr;
      instructions.get(instructionPtr).accept(registers);
      instructionPtr = (int) registers[instructionRegister];
      if (shouldHalt.test(instructionPtr, registers)) {
        break;
      }
    }
    return registers;
