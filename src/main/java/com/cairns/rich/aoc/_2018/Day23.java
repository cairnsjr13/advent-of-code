...
class Day23 extends Base2018 {
  ...
  private Table<Character, StatType, Integer> initStats(List<Bot> bots) {
    ...
    Bot.coords.forEach((coord, to) -> {
      stats.put(coord, StatType.Min, to.applyAsInt(getMin(bots, (b) -> to.applyAsInt(b))) >> INIT_SHIFT);
      stats.put(coord, StatType.Max, to.applyAsInt(getMax(bots, (b) -> to.applyAsInt(b))) >> INIT_SHIFT);
